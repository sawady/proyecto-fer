...
public class ActionCalculoAutomatico implements Action{
	...
	private List<Empleado>listaDeEmpleados =homeEmpleado.getAllEntities();
	...
	public ActionCalculoAutomatico(String ruta) throws SecurityException, IOException, FormatoEmpleadoException, NoSuchMethodException {
		super();
		this.ruta = ruta;
	}
	...
	@Override
	public void execute() {
		homeEmpleado = (EmpleadoHibernateHome) HibernateApplication
				.getInstance().getHome(Empleado.class);
		homeDeResultados = (HibernateHome <ResultadoDeCalculo>) HibernateApplication
	...
	private float deduccionesA(Empleado e){
		...
		float retorno = this.getDeduccionA().getMin_no_imp() 
						...
						* this.calculoPropHijos(e)
						+ (this.getDeduccionA().getOtros() * e.getCant_pers_a_carg_anual())
						+ calculoPersACargoProp(e)
						+ e.getGast_sepe()
						+ e.getSeg_vida()
						+ e.getServ_dom()
						+ e.getInt_cred_hip();
	...
	private float calculoConyuge(Empleado e) {
		if(e.getEstad_civil()==1){
			if(e.getMes_cas()==0){
				...
				return (this.getDeduccionA().getConyuge()/12) - (e.getMes_cas()-1);
			}
		}
		return 0;
	}
	...
	private float calculoPropHijos(Empleado e) {
		...
		if(e.getMes_nac_hij_1() != 0){
			retorno += this.propHijosAux(e.getMes_nac_hij_1(), e.getCant_hij_nac_1());
		}
		if(e.getMes_nac_hij_2() != 0){
			retorno += this.propHijosAux(e.getMes_nac_hij_2(), e.getCant_hij_nac_2());
		}
		if(e.getMes_baja_hij_1() !=0){
			retorno += this.propHijosAux2(e.getMes_baja_hij_1(), e.getCant_hij_baja_1());
		}
		if(e.getMes_baja_hij_2() !=0){
			retorno += this.propHijosAux2(e.getMes_baja_hij_2(), e.getCant_hij_baja_2());
		}
		return retorno;
	}
	...
	private float calculoPersACargoProp(Empleado e) {
		...
		if(e.getMes_alta_pers_a_carg_1() != 0){
			retorno += this.personasACargoAux(e.getMes_alta_pers_a_carg_1(), e.getCant_pers_a_carg_1());
		}
		if(e.getMes_alta_pers_a_carg_2() != 0){
			retorno += this.personasACargoAux(e.getMes_alta_pers_a_carg_2(), e.getCant_pers_a_carg_2());
		}
		if(e.getMes_baja_pers_a_carg_1() != 0){
			retorno += this.personasACargoAux(e.getMes_baja_pers_a_carg_1(), e.getCant_pers_a_carg_baja_1());
		}
		if(e.getMes_baja_pers_a_carg_2() != 0){
			retorno += this.personasACargoAux2(e.getMes_baja_pers_a_carg_2(), e.getCant_pers_a_carg_baja_2());
		}
		return retorno;
	}
	...
	private float auxGananciaNetaB(Empleado e){
		return (this.getDeduccionC().getCout_med_asist_anu()* e.getCuot_med_asist()) 
	...
	public float auxGananciaNetaC(Empleado e){
		return (e.getHonor_med() * this.getDeduccionC().getHonor_med_anu())
			+ ((((e.getImp_cheq_cred() * this.getDeduccionC().getImp_cheq_cred_anu())/100)* this.getDeduccionC().getDeb_total_imp_cheq_cred_anu())/100);
	}
	...
	public float impuestoAPagarEnElAnio(Empleado e){
		...
		return (this.getCalculo_anual().getBase() 
				...
				* (this.gananciaNetaC(e) - this.getCalculo_anual().getSobre_exced()))/100)) - this.auxImpAPagarAnio(e); 	
	}
